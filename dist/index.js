...
async function doughGet(path, params = {}) {
...
async function doughPost(path, body) {
...
async function reply(p) {
    try {
        return { content: [{ type: "text", text: await p }] };
    }
    catch (e) {
        return {
...
server.registerTool("dough_accounts", {
...
server.registerTool("dough_budget", {
...
server.registerTool("dough_net_worth", { description: "Current net worth by kind (checking, savings, investments, debts) plus the snapshot history.", inputSchema: {} }, () => reply(doughGet("net-worth")));
server.registerTool("dough_bills", { description: "Recurring bills with amount and due day of month.", inputSchema: {} }, () => reply(doughGet("bills")));
server.registerTool("dough_subscriptions", { description: "Subscriptions with amount and due day of month.", inputSchema: {} }, () => reply(doughGet("subscriptions")));
server.registerTool("dough_savings_goals", {
    description: "Active savings goals with target and derived saved amount.",
    inputSchema: { month: MONTH },
}, ({ month }) => reply(doughGet("savings-goals", { month })));
server.registerTool("dough_auto_assign_preview", {
...
server.registerTool("dough_auto_assign_apply", {
...
server.registerTool("dough_budget_assign", {
...
server.registerTool("dough_debts", { description: "Debt/loan accounts with their overrides: balance, interest rate, minimum payment, due day, original amount, notes, priority.", inputSchema: {} }, () => reply(doughGet("debts")));
server.registerTool("dough_investments", { description: "Investment accounts with their overrides: balance (market value), monthly contribution, expected return, ticker, cost basis (contributed).", inputSchema: {} }, () => reply(doughGet("investments")));
// ---- Bills (write) ----
server.registerTool("dough_create_bill", {
...
server.registerTool("dough_update_bill", {
...
server.registerTool("dough_delete_bill", { description: "Delete a bill by id (and its paid-status/history/payee patterns). Requires a write-scoped key.", inputSchema: { id: z.number().int() } }, ({ id }) => reply(doughPost("bills/delete", { id })));
// ---- Subscriptions (write) ----
server.registerTool("dough_create_subscription", {
...
server.registerTool("dough_update_subscription", {
...
server.registerTool("dough_delete_subscription", { description: "Delete a subscription by id. Requires a write-scoped key.", inputSchema: { id: z.number().int() } }, ({ id }) => reply(doughPost("subscriptions/delete", { id })));
// ---- Savings goals (write) ----
server.registerTool("dough_create_savings_goal", {
...
server.registerTool("dough_update_savings_goal", {
...
server.registerTool("dough_delete_savings_goal", { description: "Delete a savings goal by id (unlinks its category). Requires a write-scoped key.", inputSchema: { id: z.number().int() } }, ({ id }) => reply(doughPost("savings-goals/delete", { id })));
// ---- Accounts (write) ----
server.registerTool("dough_create_account", {
...
server.registerTool("dough_update_account", {
...
server.registerTool("dough_delete_account", { description: "Delete an account by id (manual accounts are removed; synced accounts are closed). Requires a write-scoped key.", inputSchema: { id: z.string() } }, ({ id }) => reply(doughPost("accounts/delete", { id })));
// ---- Debts / loans (write) ----
server.registerTool("dough_update_debt", {
...
server.registerTool("dough_reorder_debts", { description: "Set the display order of debts. Requires a write-scoped key.", inputSchema: { order: z.array(z.string()).describe("ynab_account_ids in the desired order") } }, ({ order }) => reply(doughPost("debts/reorder", { order })));
// ---- Investments (write) ----
server.registerTool("dough_update_investment", {
...
server.registerTool("dough_reorder_investments", { description: "Set the display order of investments. Requires a write-scoped key.", inputSchema: { order: z.array(z.string()).describe("ynab_account_ids in the desired order") } }, ({ order }) => reply(doughPost("investments/reorder", { order })));
// ---- Categories and targets (write) ----
server.registerTool("dough_create_category", {
...
server.registerTool("dough_update_category", {
...
server.registerTool("dough_delete_category", {
...
server.registerTool("dough_set_category_target", {
...
server.registerTool("dough_move_budget", {
...
server.registerTool("dough_snooze_category", {
...
server.registerTool("dough_unsnooze_category", {
