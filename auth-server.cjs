#!/usr/bin/env node
//
// Handles signing in, and lets nothing through to the Dough MCP without it.
...
const UPSTREAM = process.env.UPSTREAM || 'http://127.0.0.1:8470';
const ISSUER = (process.env.ISSUER || '').replace(/\/$/, '');
if (!ISSUER) {
  console.error('ISSUER is required, e.g. https://dough-mcp.rolle.wtf');
  process.exit(1);
}
...
const CONFIG_DIR = process.env.CONFIG_DIR || path.join(os.homedir(), '.config', 'dough-mcp');
...
// clients; the finance data itself is never served to a browser origin.
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  button{width:100%;margin-top:1rem;padding:.5rem;border:1px solid var(--accent);border-radius:6px;
         background:var(--accent);color:var(--on-accent);font:inherit;cursor:pointer}
  ...
  <form method="POST" action="/authorize">
      ...
      <h1>Dough</h1>
      <p>Sign in to give this app access to your Dough finances.</p>
...
app.post('/token', cors, (req, res) => {
  ...
  if (body.grant_type === 'authorization_code') {
    ...
    return res.json(issueTokens(clientId, row.resource || RESOURCE, row.scope || 'dough'));
  }
  ...
  if (body.grant_type === 'refresh_token') {
    ...
    return res.json(issueTokens(clientId, row.audience || RESOURCE, row.scope || 'dough'));
  }
...
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Dough MCP</title>
<meta name="description" content="MCP server for the Dough personal finance app.">
<link rel="icon" href="/favicon.ico" sizes="any">
<link rel="icon" href="/icon.png" type="image/png">
<link rel="apple-touch-icon" href="/icon.png">
<style>
...
app.listen(PORT, '127.0.0.1', () => {
  console.log(`dough-mcp auth server on 127.0.0.1:${PORT}`);
